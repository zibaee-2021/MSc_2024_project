...
def _impute_missing_coords(pdf_to_impute, value_to_impute_with=0):
    ...
    # pdf_to_impute[[CIF.A_Cartn_x.value, CIF.A_Cartn_y.value, CIF.A_Cartn_z.value]] =
    # (pdf_to_impute[[CIF.A_Cartn_x.value,CIF.A_Cartn_y.value, CIF.A_Cartn_z.value]]
...
def _sort_by_chain_residues_atoms(pdf: pd.DataFrame) -> pd.DataFrame:
    ...
    # pdf = pdf.sort_values(['S_seq_id', CIF.A_id.value], ignore_index=True)
...
def _reorder_columns(pdf_merged: pd.DataFrame) -> pd.DataFrame:
    ...
    return pdf_merged[[
        ...
        'S_mon_id',         # RESIDUE (3-letter)    - USE FOR SANITY-CHECK AGAINST A_label_comp_id, KEEP IN DF.
        # CIF.A_label_comp_id.value,  # RESIDUE (3-letter)    - USED TO SANITY-CHECK WITH S_mon_id, THEN REMOVE IT.
        'A_label_comp_id',  # RESIDUE (3-letter)    - USED TO SANITY-CHECK WITH S_mon_id, THEN REMOVE IT.
